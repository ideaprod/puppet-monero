...
describe 'monero' do
  ...
  describe 'with default values for parameters on' do
    platforms.sort.each do |k, v|
      context "#{k}" do
        ...
        it { should contain_class('monero') }

        it do
          should contain_group('monero').with({
            'ensure' => 'present',
            'system' => true,
          })
        end
        ...
        it do
          should contain_user('monero').with({
            'ensure'  => 'present',
        ...
        it do
          should contain_file('/run/monero').with({
            'ensure' => 'directory',
            'owner'  => 'monero',
            'group'  => 'monero',
            'mode'   => '0755',
        ...
        it do
          should contain_file('monero_config').with({
            'ensure'  => 'file',
	...
	monero_config_fixture = File.read(fixtures("monerod.conf.#{k}"))
        it { should contain_file('monero_config').with_content(monero_config_fixture) }

        it do
          should contain_file('/lib/systemd/system/monerod.service').with({
            'ensure'  => 'file',
            'owner'   => 'root',
            'group'   => 'root',
            'mode'    => '0644',
  ...
  describe 'failures' do
    ...
    context 'when major release of Debian is unsupported' do
      ...
      it 'should fail' do
        expect do
          should contain_class('monero')
        end.to raise_error(Puppet::Error, /monero supports Debian 8 \(jessie\) and Ubuntu 16.04 \(xenial\). Detected lsbdistcodename is <etch>\./)
      end
    end
    ...
    context 'when osfamily is unsupported' do
      ...
      it 'should fail' do
        expect do
          should contain_class('monero')
        end.to raise_error(Puppet::Error, /monero supports osfamilies Debian\. Detected osfamily is <Unsupported>\./)
      end
    end
  end
